...
static int pkey_bign_init(EVP_PKEY_CTX *ctx) 
{
	...
	{
		base_id = EVP_PKEY_btls_base_id(pkey);
		if (base_id == bign_nid) 
		{
...
static int pkey_bign_set_params(EVP_PKEY_CTX *ctx, EVP_PKEY *pkey) 
{
	...
	if (EVP_PKEY_btls_assign(pkey, bign_nid, key_data) <= 0)
		return 0;
	else
		return 1;
}
		...
		size_t siglen, const unsigned char *tbs, size_t tbs_len) 
{
	...
	if (pkey) 
	{
		key_data = (struct bign_key_data *) EVP_PKEY_get0(pkey);
		fill_oid_params(key_data);
		ok = (bignVerify(&key_data->params, tbs, sig, key_data->pubKey) == ERR_SUCCESS) ? 1 : 0;
	}
	return ok;
}
