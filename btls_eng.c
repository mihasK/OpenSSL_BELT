...
#ifdef _DEBUG
#pragma comment(lib, "..\\bee2\\win32\\debug\\bee2lib.lib") 
#else
#pragma comment(lib, "..\\bee2\\win32\\release\\bee2lib.lib") 
#endif

static const char *engine_btls_id = "btls";
...
const ENGINE_CMD_DEFN belt_cmds[]= {{0,NULL,NULL,0}};
...
static int bind_btls(ENGINE * e, const char *id) 
{
	...
	belt_stream.ctx_size = beltCTRStackDeep();

	belt_cfb.nid = belt_cfb_nid;
	belt_cfb.ctx_size = beltCFBStackDeep();

	belt_ctr.nid = belt_ctr_nid;
	belt_ctr.ctx_size = beltCTRStackDeep();

	belt_dwp.nid = belt_dwp_nid;
	belt_dwp.ctx_size = beltDWPStackDeep();

	belt_hash.type = belt_hash_nid;
	belt_hash.ctx_size = beltHashStackDeep();

	belt_mac.type = belt_mac_nid;
	belt_mac.ctx_size = beltMACStackDeep();
	...
	if (!ENGINE_set_cmd_defns(e, belt_cmds)) 
	{
